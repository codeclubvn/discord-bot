...
export const execute = async (interaction) => {
    ...
    return interaction.reply(`<@${target?.id}> vừa nhận được lời cảm ơn từ <@${interaction?.user?.id}>${decription ? `"${decription}"` : ''}! <@${target?.id}> đã nhận được tổng cộng **${count} lời cảm ơn**!`);
}